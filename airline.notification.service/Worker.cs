...
namespace airline.notification.service
{
    public class Worker : BackgroundService
    {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            _logger.LogInformation($"Notification Service has started at: {DateTimeOffset.Now}");            
        }

        public override async Task StopAsync(CancellationToken stoppingToken)
        {
            await _bus.StopAsync(stoppingToken);
            _logger.LogInformation($"Notification Service has stopped at: {DateTimeOffset.Now}");
        }
    }
}
